...
def lambda_handler(event, context):
    ...
    if http_method == 'GET':
        if resource =="/users":
            ...
            if isinstance(path_parameter, Error):
        ...
        if resource == "/users/{username}":
            ...
            if isinstance(parameter, Error):
                ...
                if "recommendation_id" in body.keys():
                    ...
                    result = post_list_value_for_user(dynamodb, parameter, RECOMMENDATIONS, recommendation_id, BOOKS, ID)
                    ...
                    result = post_list_value_for_user(dynamodb, parameter, READING_LIST, book_id, BOOKS, ID)
                    if isinstance(result, Error):
                        return response(result.message, 500)
                    else:
                        return response("OK", 200)
                    ...
                    result = post_list_value_for_user(dynamodb, parameter, USERS_FOLLOWING, friend_username, USERS, USER_NAME)
                    if isinstance(result, Error):
                        return response(result.message, 500)
                    else:
                        return response("OK", 200)
...
def post_list_value_for_user(dynamodb, username, list_name, value, table_name, key):
    table = dynamodb.Table(table_name)
    response = table.get_item(Key={key: str(value)})
    if "Item" not in response.keys():
        return Error("{} does not exist in the {} table.".format(key, table_name))
    
    list_result = check_if_user_has_list(dynamodb, username, list_name)
    if isinstance(list_result, Error):
        return list_result 
        
    user = get_item(dynamodb, USERS, {USER_NAME: username})
    if value in user[list_name]:
        return Error("{} already exists in {} for {}".format(key, list_name, username))
